...
public class BtcReleaseClientServiceImplTest {
    ...
    @Test
    public void hasBtcTxHash_ok_found_in_pegout_creation_index() {
        Sha256Hash sha256Hash = TestUtils.createBtcTransaction(params, federation).getHash();
        Keccak256 hash = TestUtils.createHash(5);
        hasBtcTxHash( sha256Hash, hash, true, false);
    }

    @Test
    public void hasBtcTxHash_ok_found_in_pegout_creation_index_and_exist_in_legacy_storage() {
        Sha256Hash sha256Hash = TestUtils.createBtcTransaction(params, federation).getHash();
        Keccak256 hash = TestUtils.createHash(15);
        hasBtcTxHash( sha256Hash, hash, true, true);
    }

    @Test
    public void hasBtcTxHash_found_in_legacy_storage() {
        Sha256Hash sha256Hash = TestUtils.createBtcTransaction(params, federation).getHash();
        Keccak256 hash = TestUtils.createHash(35);
        hasBtcTxHash( sha256Hash, hash, false, true);
    }

    @Test
    public void hasBtcTxHash_not_found() {
        Sha256Hash sha256Hash = TestUtils.createBtcTransaction(params, federation).getHash();
        Keccak256 hash = TestUtils.createHash(25);
        hasBtcTxHash( sha256Hash, hash, false, false);
    }
    ...
    private void hasBtcTxHash(Sha256Hash sha256Hash, Keccak256 hash, boolean fromPegoutCreationIndex, boolean fromLegacyStorage) {
        ...
        Assert.assertEquals(
            fromPegoutCreationIndex || fromLegacyStorage,
        ...
        if (fromLegacyStorage && !fromPegoutCreationIndex){
            verify(btcReleaseClientStorageAccessor, times(1)).hasBtcTxHash(sha256Hash);
        }
    }
